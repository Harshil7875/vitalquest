...
- **Right to Erasure is a two-step flow:** `health_service` hard-deletes `BiometricLog` rows, then publishes an `ErasureEvent` so `game_service` anonymizes its Redis state to `deleted_user_{hash}`. Both halves must run for a deletion to be complete.

## Service-by-service quick map

- **`health_service/`** — FastAPI on `:8000`. Routers: `auth`, `health`, `webhooks`, `clinical`. Background jobs run via APScheduler in `tasks/` (guild aggregation cron at 23:59 UTC, OAuth token rotation every 50 min). Adapters in `adapters/` translate provider payloads (Apple HealthKit, Google Fit, Dexcom, …) into the internal `VitalQuestStandardPayload`. Reward events are emitted by `publisher/redis_publisher.py`.
